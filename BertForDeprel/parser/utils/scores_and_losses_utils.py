...
def compute_loss_deprel(deprel_scores_pred: Tensor, deprels_true: Tensor, heads_true, criterion: CrossEntropyLoss):
    """
...
def compute_loss_class(class_pred: Tensor, class_true: Tensor, criterion: CrossEntropyLoss):
    ...
    class_pred: (batch_len {index of sentence in batch}, seq_len {token index}, num_classes {score
    assigned to each class})
    class_true: (batch_len {index of sentence in batch}, seq_len {index of true class label}})
    criterion: the loss function to perform forward propagation with
    """
...
def compute_acc_deprel(deprel_scores_pred, deprels_true, heads_true):
    ...
    mask = __mask_from_dummies(heads_true)
    total = __sum_2d_tensor(mask)
    deprel_scores_pred_for_true_heads = _deprel_pred_for_heads(deprel_scores_pred, heads_true)
...
def compute_LAS(head_scores_pred: Tensor, deprel_scores_pred: Tensor, heads_true: Tensor, deprels_true: Tensor):
...
def compute_LAS_chuliu(heads_pred: Tensor, deprel_scores_pred: Tensor, heads_true: Tensor, deprels_true: Tensor):
    """The difference between this and compute_LAS is only in how the head predictions are
    provided; this function accepts predicted head indices, while the other accepts scores
    over all possible heads for each dependent.
    heads_pred: tensor of dimensions (batch_len, seq_len) containing the predicted head index for
    each dependent."""
    deprel_scores_pred_for_true_heads = _deprel_pred_for_heads(deprel_scores_pred, heads_true)
